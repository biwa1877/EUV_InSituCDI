function [dynamic,static,err] = inSitu_RAAR_Recon_GPU_active_v04(ESWft,maskStatic,maskDynamic,static,probe,dynamic,Ni,sphs0,gamma,beta,eta1,eta2,plotFreq)
%
...
Pratio = abs(probe) / max(abs(probe(:))) .* conj(probe)./(abs(probe).^2+eps);
...
for ii = 1:Ni
    % break
    if strcmp(get(gcf,'currentCharacter'),'b'), clc; break; end
    for tt = randperm(Nt)%1:Nt
...
%         StP = static(:,:,1);
        ...
        if ii > 1
            % overlap reflector
            psi_OR = (1 + eta2) * psi_OP(:,:,tt) - eta2 * psi;
            % RAAR
            psi = 0.5 * beta * (psi_OR + psi) + (1 - beta) * psiP(:,:,tt);
        end
...
%         psiP = fftshift(ifft2(ifftshift(PsiP))).*conj(sphs0)*norm;
        psiP(:,:,tt) = ifft2(PsiP) .* conj(sphs0) * norm;
        
        psi_RM = (1 + eta1) * psiP(:,:,tt) - eta1 * psi;
%         psiDel = psiP-psi;
        psiDel = psi_RM - psi;
        ...
        dynamic(:,:,tt) = objectP .* (1-maskStatic);
%         dynamic(:,:,tt) = objectP .* maskDynamic;
        
        % overlap projector
        psi_OP(:,:,tt) = (static(:,:,tt) + dynamic(:,:,tt)) .* probe;
